...
**Las dos bandas bajo la barra superior, y por qué una se cierra y la otra no.** El cierre no es una preferencia de estilo: lo decide qué se pierde al descartar. La falla de almacenamiento (`error`) **no lleva cierre**, porque descartarla deja al usuario trabajando en la creencia de que se está guardando, que es justo la pérdida de trabajo que la banda existe para evitar; su única salida es exportar el JSON. El aviso de versión nueva (`info`) **sí se cierra**, porque posponerlo no cuesta nada, y el cierre recuerda el número visto para que la versión siguiente vuelva a avisar. Cuando las dos coinciden, la urgente va arriba.
...
- **Barra superior:** identidad del proyecto a la izquierda, testigo de guardado ("Guardando" / "Guardado 12:40", en registro de rótulo, sin icono y sin animación), lente de unidad y salidas del trabajo a la derecha. La **versión de la copia** ("v 0.1.0") va pegada a la marca, en el mismo registro de rótulo que el testigo, con la fecha de compilación en el `title`. No es un adorno de pie de página: el entregable se copia a mano de un computador a otro y este número es lo único que distingue dos copias sobre la misma mesa. Bajo 900px se retira, porque es dato de identificación y no de trabajo.
...
- **Dos hojas.** El **Consolidado** es la lista de compra: lo que quedó fuera arriba, la tabla, y la composición por recinto bajo cada línea. El **Recinto** es la lámina de terreno: la planta con su despiece y sus cotas, el listado y la memoria de cálculo de cada línea.
- **En papel el Consolidado NO lleva dinero.** Ni precio unitario, ni subtotal, ni total del proyecto. La hoja se le entrega al encargado de adquisiciones, que necesita qué comprar y cuánto; un precio impreso envejece en el bolsillo y se discute en el mesón contra la cotización real. El presupuesto sigue completo en pantalla, que es donde se presupuesta, y `composicionDeGrupo` apaga su cola de dinero con `conDinero: false` para que el costo no se cuele escrito dentro de la composición. Decisión del usuario, agosto 2026.
- **El espacio del dinero se le devuelve al comprador.** La columna de precio unitario pasa a ser una **casilla** de verificación al borde izquierdo —el cuadrado del vocabulario impreso, filete de control de un píxel, cero radio: es tinta que se marca a lápiz, no un control—, y la de subtotal pasa a ser una **columna en blanco con renglón**, rotulada «Proveedor y precio», donde se anota lo que Kubikar no sabe y no debe inventar. El renglón se apoya **abajo** en la celda, no arriba: una fila con dos líneas de composición es alta, y un renglón anclado al techo deja escribiendo en el aire. El pie de tabla cuenta los materiales de la lista —una lista de compra se dobla en un bolsillo y se separa— y bajo la tabla cierran dos renglones de recepción, «Recibe conforme» y «Fecha».
