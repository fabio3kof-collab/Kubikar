...
- Biblioteca de materiales propia y compartida entre proyectos, con tipos Plancha, Barra o perfil, y Pieza o accesorio. Se precarga con materiales típicos del mercado chileno, editables. La barra declara además dos datos de corte: el traslapo que consume cada empalme y el retazo mínimo aprovechable, bajo el cual el sobrante es descarte y no se reutiliza en otra corrida. Son propiedad del perfil, no del recinto, porque un omega y un ángulo perimetral se comportan distinto y el dato se edita una vez.
...
- Método de cálculo: la cantidad sale de la planta. Los ejes de perfilería se recortan contra el polígono y entregan corridas de largo real; el perfil perimetral usa los lados del polígono; la plancha cuenta las posiciones de la retícula que tocan el recinto; los accesorios siguen saliendo de la densidad por metro cuadrado. Sobre eso se aplica el porcentaje de desperdicio, que cubre rotura y error de corte y ya no carga con el retazo. Cada resultado muestra la cantidad teórica antes del redondeo y del desperdicio.
...
- El lienzo dibuja el reparto de planchas y de ejes de perfilería dentro del polígono, como referencia de replanteo para el trabajo en terreno. Decisión del usuario, agosto 2026. **Es el mismo dato del que sale la cubicación**: el módulo resuelve el reparto una sola vez y de ahí leen tanto las cantidades como el dibujo, de modo que el número que se compra y la retícula que se ve no pueden divergir. Sigue sin numerar cortes ni rotular piezas. El módulo lo declara con `trazar(ctx)` y el lienzo lo recorta contra la planta; la orientación la define el parámetro "Dirección del perfil", que ahora sí puede mover las cantidades, porque cambia cómo se reparten las corridas y las posiciones. Se apaga con dos interruptores independientes en la barra del lienzo, junto a "ajustar vista" —uno para las piezas de material y otro para los ejes de perfilería, rotulados por el propio módulo—: arrancan encendidos y la elección se recuerda entre sesiones, igual que el imán y el modo ortogonal.
