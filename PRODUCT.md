...
3. Dibuja el polígono del recinto en el lienzo de grilla. Cada clic agrega un vértice y traza un segmento desde el vértice anterior, como una polilínea. El polígono se cierra al hacer clic sobre el primer vértice o con el botón "Cerrar polígono". Mientras la figura está abierta no se puede calcular.
   ...
   - Dos detentes gobiernan el trazo y se apagan por separado. El **imán** hace dos cosas, porque en la mano del que dibuja son una sola: pega el vértice a la grilla y pega el trazo a los múltiplos de 15°, que son los ángulos que aparecen en una planta —0, 30, 45, 60, 90—. Es imán y no jaula: la dirección se pega solo si viene a menos de 5° de un detente, así que un muro a 22° se dibuja igual con el imán puesto. Cuando la dirección se imanta, el largo se pega SOBRE ESE RAYO y no a la grilla absoluta, que es la única forma de que las dos ayudas no se peleen y de que una horizontal siga siendo horizontal cuando arranca de un vértice fuera de grilla. El **modo ortogonal** es el detente duro de 90°, para que un recinto rectangular no pueda salir torcido, y manda sobre el angular. Decisión del usuario, agosto 2026.
...
7. Guarda el proyecto en el PC como archivo y exporta el listado de materiales en CSV.
...
- Método de cálculo: la perfilería sale de la planta. Los ejes se recortan contra el polígono y entregan corridas de largo real; el perfil perimetral usa los lados del polígono. La plancha es la excepción y se cubica por los metros cuadrados que cubre —área neta del recinto dividida por el área útil de la plancha, o sea su ancho y su largo menos el traslapo—, porque contar las posiciones de la retícula que tocan el recinto cobraba entera una posición que entraba cinco centímetros y sobredimensionaba la compra; en obra ese rendimiento se recupera cortando, y lo que el corte no recupera lo cubre el desperdicio, que por eso arranca en 10% para las planchas y en 5% para el resto. Decisión del usuario, agosto 2026. Los tornillos son dos y se cuentan donde van: el drywall punta fina sobre los metros lineales de perfilería, que es donde se atornilla la plancha, y el punta broca cabeza de lenteja por punto de unión metal-metal, o sea los dos extremos de cada corrida más un tornillo por colgante. Solo el colgante sale de una densidad por metro cuadrado. Los dos tornillos declaran además un escalón de compra, siempre hacia arriba y solo ellos: bajo 50 unidades se lleva 50, y de ahí en adelante se sube a la centena siguiente —65 se compra 100, 101 se compra 200, 358 se compra 400, 1.150 se compra 1.200—. Existe porque un tornillo no se despacha de a uno: quien llega a la ferretería con 358 redondea igual, pero en el mesón y sin que quede escrito en la cubicación. **El escalón se aplica en la lista de compra y no en el recinto**, y esa es la decisión de fondo: el recinto muestra la cantidad exacta que lleva —20 unidades son 20 unidades, y así se instalan— y el consolidado suma los recintos y sube el total una sola vez. Tres recintos de 20 tornillos son 60 tornillos y una sola visita a la ferretería, así que se piden 100; redondear en cada recinto los convertiría en 150, que es material comprado de más por redondear tres veces lo que se compra una. Es la excepción a que el consolidado sume cantidades ya redondeadas, y por la misma razón que esa regla: el retazo de una barra no viaja al recinto siguiente, pero un tornillo sí. El múltiplo exacto no gasta un escalón de más, la composición del grupo declara de dónde a dónde subió el total, y el costo se cobra sobre lo que se compra. Decisión del usuario, agosto 2026. Sobre eso se aplica el porcentaje de desperdicio, que cubre rotura y error de medida: en la barra ya no carga con el retazo, que se cuenta aparte en el reparto, y en la plancha sí carga con el corte contra los bordes del recinto, que es lo que la cubicación por área no cuenta. Cada resultado muestra la cantidad teórica antes del redondeo y del desperdicio.
...
- Guardar y abrir el proyecto como archivo en el PC, con esquema JSON estable y versionado. Las dos acciones viven juntas en la barra superior y se rotulan por su destino —"Guardar en el PC", "Abrir desde el PC"— y no por su formato: lo que el usuario decide ahí es dónde queda su trabajo. Abrir nunca pisa lo guardado; si el identificador colisiona, el proyecto entrante recibe uno nuevo. Decisión del usuario, agosto 2026: la salida existía sola desde la primera versión y su entrada estaba dos clics adentro, lo que hacía creer que Kubikar no sabía leer sus propios archivos.
- Guardar y abrir la biblioteca de materiales completa como archivo propio, separado del proyecto. Existe porque la biblioteca es compartida entre proyectos y el archivo de proyecto lleva solo los materiales que ese proyecto usa: sin esto, mudar a otro computador un catálogo de perfiles con sus precios obliga a recargarlo a mano. Abrir MEZCLA y nunca reemplaza —un material que ya está, mismo tipo y mismo nombre, se omite con sus datos intactos— y la operación declara cuántos entraron y cuántos ya estaban. Decisión del usuario, agosto 2026.
- Impresión en papel de dos documentos, con botón propio en cada vista: el consolidado como lista de compra —lo que quedó fuera, la tabla y la composición por recinto—, y el recinto como lámina de terreno —la planta con su despiece y sus cotas, el listado de materiales y la memoria de cálculo de cada línea—. Decisión del usuario, agosto 2026.
- El consolidado impreso no lleva dinero: ni precio unitario, ni subtotal, ni total del proyecto. Es un papel que se le entrega al encargado de adquisiciones, que necesita saber qué comprar y cuánto, no cuánto costaba en la biblioteca el día que se cubicó; un precio impreso envejece en el bolsillo y se discute en el mesón contra la cotización real. El presupuesto con sus cifras sigue completo en pantalla, que es donde se presupuesta. El espacio que ocupaba el dinero se le devuelve al comprador: una casilla para marcar lo ya comprado o recibido, una columna en blanco con renglón rotulada "Proveedor y precio" para anotar lo que Kubikar no sabe, el conteo de materiales al pie —una lista de compra se dobla en un bolsillo y se separa— y dos renglones de recepción, "Recibe conforme" y "Fecha". Decisión del usuario, agosto 2026.
